...
def scenario_failed_restore_cleans_snapshot() -> ScenarioResult:
    env = TestEnv("release_e2e_failed_restore_cleanup")
    env.setup()
    result = ScenarioResult(name="failed-restore-cleans-snapshot", success=False, errors=[])
    port = get_free_port()
    ...
    try:
        initialize_db(env.primary_db)
        write_rows(env.primary_db, 0, 10)
        ...
        replica, replica_dir = start_replica(
            env,
            "replica_restore_cleanup",
            port,
            force_restore=True,
        )
        blocking_path = replica_dir / "primary.db"
        blocking_path.mkdir(exist_ok=True)

        exit_code = replica.proc.wait(timeout=60)
        replica.log_handle.flush()

        if exit_code == 0:
            result.errors.append("Replica restore failure scenario exited successfully")
            return result

        staged = list(meta_dir(replica_dir).glob("snapshot_*.zst"))
        if staged:
            result.errors.append(f"Staged snapshot artifacts leaked: {staged}")

        log_text = replica.log_path.read_text()
        if "restore snapshot failed" not in log_text:
            result.errors.append("Replica log did not record restore snapshot failure")
...
SCENARIOS = {
    ...
    "failed-restore-cleans-snapshot": scenario_failed_restore_cleans_snapshot,
}
